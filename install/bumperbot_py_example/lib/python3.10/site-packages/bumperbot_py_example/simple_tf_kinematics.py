...
class SimpleTfKinematics(Node):
    def __init__(self):
        super().__init__("simple_tf_kinematocs")
        ...
        #defining how those two frame are connected to one another
        ...
        self.timer_ = self.create_timer(0.3,self.timerCallback)
        self.get_transform_srv_ = self.create_service(GetTransform,"get_transform",
                                                      self.getTransformCallback)
    ...
    def getTransformCallback(self,req,res):
        self.get_logger().info("Requested transform between %s and %s"%(
        ...
        try:
            ...
            self.get_logger().error("An error occur while transfroming between %s and %s"%(
                req.frame_id,req.child_frame_id))
            res.success = False
            return res
        res.transform = requested_transform.transform
        res.success = True
        return res
